# This is the query by encounter_datetime. It helps to estimate the number of forms we expected to be entered for the current month.
# Paste the result into the worksheet: "encounter_data"
(select 
	...
	max(e.date_created) as max_date_created,
...
# This is the query by date_created. It helps to assess how many forms have been completed by the data entry team
# Paste the result into the worksheet: "data_entry_data"
...
(select 
	...
	month(e.date_created) as month,
	e.creator,
	concat(p.given_name,' ',p.middle_name,' ',p.family_name) as name,
	max(encounter_datetime) as max_encounter_datetime,
	max(e.date_created) as max_date_created,
	count(*) as all_encounters,
	count(if(encounter_type in (1,2,3,4,10,13,14,15,17,19,22,23,26,43,47,21),1,null)) as hiv_encounters, 
	...
	count(if(encounter_type=47,1,null)) as pmtct_postnatal
from amrs.encounter e
	join amrs.person_name p on e.creator = p.person_id 
where e.date_created >= (DATE_FORMAT(NOW() ,'%Y-%m-01') - interval 1 year) #Better to include voided as this represents work done
	and day(e.date_created) < day(curdate())
	and e.date_created between (DATE_FORMAT(NOW() ,'%Y-%m-01') - interval 1 year) and curdate()
group by year,month,e.creator
order by e.creator,year desc,month desc);

...
proc sort data=identifier8; by patient_id preferred; run;
...
proc sort data=identifier8 nodupkey out=dup1 ; by patient_id; run;


/* get those who have ampath Id and not old ampath id nor universal id*/
data ident;
merge identifier3(in=a) identifier1(in=b) identifier8(in=c);
by patient_id;
if b and (not a or c);
run;

proc sort data=ident; by patient_id preferred; run;
/* get those who have universal id  and not old ampath id nor ampath Id */
...
proc sort data=ident21; by patient_id preferred; run;

...
%MACRO create_Reason_stop_drugs();	

PROC IMPORT OUT= WORK.stopstartarv
            DATAFILE= "C:\DATA\DATA SETS\Reason_stop_or_start_arv.csv" 
            DBMS=CSV REPLACE;
     GETNAMES=YES;
     DATAROW=2; 
RUN;
...
data stopstartarv1;
format start_stop_date stop_date start_date ddmmyy10.;
set stopstartarv;
...
*keep person_id app_date location_id Tx_failure  ;
...
proc sort data=stopdrug1 out=stopdrug_final nodupkey; by person_id stop_date ; run;


PROC DATASETS LIBRARY=WORK NOLIST;
		DELETE stopdrug stopdrug1 ;
		RUN;
   		QUIT;


%MEND create_Reason_stop_drugs;
%create_Reason_stop_drugs;

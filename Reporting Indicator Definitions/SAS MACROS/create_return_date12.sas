...
set rtc.Return_visit_date1 rtc.Return_visit_date2;
run;
...
proc sort data=rtc nodupkey; by person_id app_date encounter_id;
proc sort data=return_date1; by person_id app_date encounter_id;

Data check_dates;
merge rtc(in=a) return_date1(in=b);
...
merge return_date1(in=a) check_dates(in=b);
by person_id app_date;
if a or b;
if rtc_period='weeks' and value_numeric ne . then estimated_rtc=app_date+(value_numeric*7);
else if rtc_period='months' and value_numeric ne . then estimated_rtc=app_date+(value_numeric*30.25);
else if rtc_period='days' and value_numeric ne . then estimated_rtc=app_date+value_numeric;
run;

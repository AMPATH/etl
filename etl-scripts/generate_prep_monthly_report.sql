CREATE DEFINER=`fmaiko`@`%` PROCEDURE `generate_prep_monthly_report`(IN query_type varchar(50), IN queue_number int, IN queue_size int, IN cycle_size int , IN log boolean)
BEGIN
	select @start := now();
	select @table_version := "prep_monthly_report_dataset_v1_1";
	set @primary_table := "prep_monthly_report_dataset_v1_1";
	...
	CREATE TABLE IF NOT EXISTS `prep_monthly_report_dataset_v1_1` (
		...
		KEY `person_id_2` (`person_id`,`month`),
		KEY `month` (`month`),
		KEY `location_id` (`location_id`,`month`),
	...
	while @person_ids_count > 0 do
		...
		(index (person_id), index(person_id, month, encounter_id))
		( 
			select * from (select 
				*
				from 
		...
		ORDER BY h.person_id , month, h.encounter_datetime desc , rtc_date
		) p group by person_id, month);
		...
		( 
			...
			index (person_id),  
			index (person_id, month), 
			index(month), 
			index(location_id, month), 
			index(encounter_datetime))
		(SELECT
			concat(month, person_id) as elastic_id,
			location_id,
			person_id, 
			uuid AS person_uuid,
			...
			CASE
				WHEN @encounter_month = month AND year(encounter_datetime) = year THEN @visit_this_month := 1
				ELSE @visit_this_month := 0 
			END AS visit_this_month,
			
			CASE
				WHEN @prev_rtc_month = month AND year(prev_rtc_date) = year THEN @appointment_this_month := 1
				WHEN @rtc_month = month AND year(rtc_date)= year THEN @appointment_this_month := 1
			...
			IF(@visit_this_month = 0  AND @appointment_this_month = 1,1,0) AS missed_appointment_this_month,
				...
				THEN @status:='dead'
				WHEN 
					month >= month(discontinued_prep_date) 
					THEN 
					@status:='discontinued'
				WHEN
					@days_since_rtc_date <= 0
				THEN
					@status:='active'
				WHEN
					@days_since_rtc_date > 0  AND @days_since_rtc_date <= 28
				THEN
					@status:='defaulter'
				WHEN
					@days_since_rtc_date > 28
				THEN
					@status:='ltfu'
				ELSE @status:='unknown'
			END AS status,
			...
			if(month(enrollment_date) = month AND year(enrollment_date) = year, 1, 0) as enrolled_in_prep_this_month,
			if(month(discontinued_prep_date) = month AND year(discontinued_prep_date) = year, 1, 0) as discontinued_from_prep_this_month,
			case when month(turned_positive_date) = month AND year(turned_positive_date) = year then @turned_positive_this_month = 1
			else @turned_positive_this_month = 0  end as turned_positive_this_month,
			...
			replace into prep_monthly_report_dataset_v1_1
		(select
			*
			from stage_1);

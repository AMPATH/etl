...
arveverchanged(keep=person_id arv_date arveverchanged) ;
 ...
 if (concept_id=1192 and value_numeric='1') or (concept_id=1255 and value_coded in('981','1256','1257','1258','1259','1406','1849','1850','1260'))
or (concept_id=1088 and value_coded ne '') or (concept_id=966 and value_coded ne '') or (concept_id=1252 and value_coded ne '') or 
(concept_id in (1250,1251) and value_coded ne '') or (concept_id=2154 and value_coded not in ( '1066','')) or (concept_id=1999 and value_coded='1065') or
(concept_id=1499 and arv_date ne .)  or (concept_id in (1086, 1087,1387) and value_coded not in ('1067','1624','1066')) then onarv=1;
...
if concept_id=1252 and value_coded ne ' ' then arvreasonstop=1 ; 
...
data arvuse;
merge  onarv(in=a) arvchangeformulation arvchangeregimen arvstopall arvstart  arvdosechange arvrestart arvsubstitution arveverchanged ;
...
data stav(keep=person_id  arv_date stavudine) lam(keep=person_id  arv_date lamivudine) 
	...
	Ataza(keep=person_id  arv_date ATAZANAVIR)
	unk(keep=person_id  arv_date unk) other(keep=person_id  arv_date other);
  ...
  else if value_coded in ('749') then do ;
	INDINAVIR=1 ;
	output ind ;
  end ;
 ...
 else if value_coded='6157' then do ;
	DARUNAVIR=1 ;
	output Dar ;
  end ;
else if value_coded='6158' then do ;
	ETRAVIRINE=1 ;
	output etra ;
  end ;

else if value_coded='6159' then do ;
	ATAZANAVIR=1 ;
	output ataza ;
  end ;
...
data arv_final;
set arv_final1 /*arv_firstdrugs(keep=person_id  arv_date firstdrug)*/;
by person_id arv_date ;
IF firstdrug=1 THEN arvstart=1;
...
PROC DATASETS LIBRARY=WORK NOLIST;
		DELETE arvs onarv arvchangeformulation arvchangeregimen arvstopall Arvreasonstop arvstart arvdosechange arvrestart arvsubstitution arveverchanged
		stav1 lam1 nev1 efav1 nelf1 lop1 rit1 did1 azt1 ten1 abac1 ind1 emtri1 ralt1 dar1 etra1 ataza1  arv_final1 UNK1 OTHER1
		stav lam nev efav nelf lop rit did azt ten abac ind emtri ralt dar etra ataza UNK OTHER arvuse arvmeds mednotuse usestmed usenotmed arv_firstdrugs;
		RUN;
   		QUIT;

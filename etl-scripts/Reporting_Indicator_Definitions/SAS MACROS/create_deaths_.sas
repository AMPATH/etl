...
proc sort data=encounters ;by person_id encounter_date;run;
...
data deaths_f deaths_dqa;
merge death_combined_all(in=a) last_visits(in=b) ;
by person_id;
if a then output deaths_f;
if a and encounter_date>reported_date and encounter_date ne . and encounter_date>dod then output deaths_dqa;
run;

data identifiers;
set Identifier_final;
keep person_id identifier

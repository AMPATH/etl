...
from amrs.obs where voided=1 and date_voided > @last_obs_date_created and date_created <= @last_obs_date_created and concept_id in (1592,9081,9062,9063,9067,9069,1558,1553,1559,1552,1816,9073,1568,1569,1733,1570,1573,1588,9077,9082,1583,5096) );
...
where o.date_created > @last_obs_date_created and o.voided=0 and e.date_created <= @last_enc_date_created and e.voided=0 and concept_id in (1592,9081,9062,9063,9067,9069,1558,1553,1559,1552,1816,9073,1568,1569,1733,1570,1573,1588,9077,9082,1583,5096) and encounter_type=21
);
...
(select * from amrs.obs o where concept_id in (1592,9081,9062,9063,9067,9069,1558,1553,1559,1552,1816,9073,1568,1569,1733,1570,1573,1588,9077,9082,1583,5096) and o.voided=0 and date_created > @last_obs_date_created);

# add obs of encounters with voided obs
insert ignore into obs_subset
(select t1.* from amrs.obs t1 join voided_obs t2 using (encounter_id) where t1.voided=0 and t1.date_created <= @last_obs_date_created and t2.encounter_id is not null and t1.concept_id in (1592,9081,9062,9063,9067,9069,1558,1553,1559,1552,1816,9073,1568,1569,1733,1570,1573,1588,9077,9082,1583,5096));

# add obs for encounters which have new obs
insert ignore into obs_subset
(select o.* from amrs.encounter e join amrs.obs o using (encounter_id) where o.date_created > @last_obs_date_created and o.voided=0 and e.date_created <= @last_enc_date_created and e.voided=0 and concept_id in (1592,9081,9062,9063,9067,9069,1558,1553,1559,1552,1816,9073,1568,1569,1733,1570,1573,1588,9077,9082,1583,5096));

...
(select 
encounter_id,
case concept_id=1941
	when value_coded in (6467,6964,792,633,631) then 1
	when value_coded = 794 then 2
	when value_coded = 6156 then 3 
	else null
end as arv_regimen,
if(concept_id in (5096,1502,1777),value_datetime,null) as rtc_date,
if(concept_id in (1042,1040) and value_coded=703,1,0) as enc_tested_hiv_positive,
if(concept_id=1255,value_coded,null) as arv_plan,
case
...
if(concept_id=856, value_numeric,null) as enc_vl,
if(concept_id=5497,value_numeric,null) as enc_cd4_count, 
if(concept_id=730,value_numeric,null) as enc_cd4_percent,

# outreach follow-up form specific section
if(concept_id = '',value_numeric,null) as call_attempts,
if(concept_id = '',value_coded,null) as found_patient,
if(concept_id = '',value_coded,null) as refer_for_field_follow_up,
if(concept_id = '',value_coded,null) as field_follow_up_not_attempted,
if(concept_id = '',value_coded,null) as public_means_attempt_number,
if(concept_id = '',value_coded,null) as ccc_attempt_number,
if(concept_id = 1816,value_numeric,null) as days_of_arvs_remaining,
if(concept_id = '',value_coded,null) as final_attempt,
if(concept_id = '',value_datetime,null) as date_found,
if(concept_id = '',value_coded,null) as location_of_contact,
group_concat(if(concept_id=1733,value_coded,null) separator ' ') as reason_for_missed_appt,
if(concept_id = '',value_coded,null) as reason_for_death,
if(concept_id = '',value_coded,null) as reason_refused_care,
if(concept_id = '',value_coded,null) as likelihood_patient_rtc,
if(concept_id = 'ampath status concept_id', value_coded,null) as ampath_status,
if(concept_id = '',value_coded,null) as referred_for_info_update,
max(date_created) as obs_date_created
from amrs.obs o
where concept_id in (1941,5096,1502,1777,1042,1040,1255,1250,856,5497,730) and o.voided=0 and encounter_id is not null
...
/*
case concept_id = 'AMPATH Status concept_id'
	when value_coded = 'in care' then 1
	when value_coded = 'dead' then 2
	when value_coded = 'self-disengaged' then 3
	when value_coded = 'hiv negative' then 4
	when value_coded = 'transferred to non-amrs ampath clinic' then 5
	when value_coded = 'transferred to amrs ampath clinic' then 6
	when value_coded = 'transferred to non-ampath clinic' then 7
	when value_coded = 'untraceable' then 8
	when value_coded = 'tracing in process' then 9
*/
...
(select p.person_id, p.gender, p.birthdate from amrs.person p where p.voided=0);
...
if(@prev_id = @cur_id,
	if((encounter_datetime < @prev_hiv_start_date or @prev_hiv_start_date is null) and (tested_hiv_positive or encounter_type in (1,2,3,4,10,11,12,14,15,17,19,26,44,46,47) or arv_regimen), @hiv_start_date := encounter_datetime,@prev_hiv_start_date),
	if(tested_hiv_positive or encounter_type in (1,2,3,4,10,11,12,14,15,17,19,26,44,46,47) or arv_regimen,@hiv_start_date := encounter_datetime, @hiv_start_date:=null)) as hiv_start_date,

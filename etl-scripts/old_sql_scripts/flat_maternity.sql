...
(select * from amrs.obs o use index (date_created) where concept_id in (1856,5272,1363,1846,1992,2055,2198,5630,1836,5596,5599,1279,1855,5992) and o.voided=0 and date_created > @last_update);
...
insert ignore into obs_subset
(select o.* from amrs.encounter e join amrs.obs o use index (date_created) using (encounter_id) where o.date_created > @last_update and o.voided=0 and e.date_created <= @last_update and e.voided=0 and o.concept_id in (1856,5272,1363,1846,1992,2055,2198,5630,1836,5596,5599,1279,1855,5992));

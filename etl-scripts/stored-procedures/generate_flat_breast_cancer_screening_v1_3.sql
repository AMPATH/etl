DELIMITER $$
CREATE PROCEDURE `generate_flat_breast_cancer_screening_v1_3`(IN query_type VARCHAR(50), IN queue_number INT, IN queue_size INT, IN cycle_size INT)
BEGIN
    -- v1.3:  Rename `breast_complaints_3months` column to `breast_complaints_3months`
    --        Rename `Ultrasound_results` to `ultrasound_results`
    --        Add new columns: 
    ...
    SET @start := NOW();
    SET @table_version := "flat_breast_cancer_screening_v1.3";

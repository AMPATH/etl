...
-- 							end as prep_start_date_branch,
                            ...
                            case
							   ...
							   case
								when obs regexp "!!9754=5566!!" then @initiation_reason := 'PARTNER CONFIRMED HIV+'
                                when obs regexp "!!9754=7903!!" then @initiation_reason := 'MORE THAN ONE SEXUAL PARTNER'
                                when obs regexp "!!9754=174!!" then @initiation_reason := 'SEXUALLY TRANSMITTED INFECTION'
                                when obs regexp "!!9754=7104!!" then @initiation_reason := 'CONDOM NOT USED'
                                when obs regexp "!!9754=9760!!" then @initiation_reason := 'ONGOING GENDER BASED VIOLENCE'
                                when obs regexp "!!9754=9762!!" then @initiation_reason := 'SEX PARTNER HIGH RISK FOR HIV AND HIV STATUS UNKNOWN'
                                when obs regexp "!!9754=9763!!" then @initiation_reason := 'TRANSACTIONAL SEX'
                                when obs regexp "!!9754=9764!!" then @initiation_reason := 'RECURRENT USE OF PEP'
                                when obs regexp "!!9754=9765!!" then @initiation_reason := 'RECURRENT SEX'
                                when obs regexp "!!9754=105!!" then @initiation_reason := 'INTRAVENOUS DRUG USE'
                                when @prev_id = @cur_id then @initiation_reason
                                else @initiation_reason := null
							end as initiation_reason,

							 case
								when obs regexp "!!1596=1169!!" then @discontinued_reason := 'HIV INFECTED'
                                when obs regexp "!!1596=9777!!" then @discontinued_reason := 'LOW RISK OF HIV'
                                when obs regexp "!!1596=1885!!" then @discontinued_reason := 'RENAL FAILURE'
                                when obs regexp "!!1596=9778!!" then @discontinued_reason := 'CLIENT REQUEST '
                                when obs regexp "!!1596=1434!!" then @discontinued_reason := 'POOR ADHERENCE, NOS'
                                when obs regexp "!!1596=9779!!" then @discontinued_reason := 'VIRAL SUPPRESSION OF HIV POSITIVE PARTNER'
                                when obs regexp "!!1596=9781!!" then @discontinued_reason := 'TOO MANY HIV TESTS'
                                when obs regexp "!!1596=5622!!" then @discontinued_reason := 'OTHER NON-CODED '
                                when @prev_id = @cur_id then @discontinued_reason
                                else @discontinued_reason := null
							end as discontinued_reason

DELIMITER $$
CREATE PROCEDURE `generate_hiv_monthly_report_dataset_v1_1`(IN query_type varchar(50), IN queue_number int, IN queue_size int, IN cycle_size int, IN start_date varchar(50))
BEGIN
...
#						when transfer_out_date < date_format(endDate,"%Y-%m-01") then @status := "transfer_out"
					...
					case when arv_first_regimen_location_id is not null then arv_first_regimen_location_id 

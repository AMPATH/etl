DELIMITER $$
CREATE  PROCEDURE `generate_rebuild_hiv_summary`()
BEGIN
                    select @_query_type := "sync"; 
					select @_start := now();
					select @_start := now();
					select @_table_version := "rebuild_flat_hiv_summary_v2.12";
					...
					while @_person_ids_count > 0 do
						...
						create temporary table rebuild_hiv_summary_queue_0 (select * from rebuild_hiv_summary_queue limit 1000); 

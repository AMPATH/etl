DELIMITER $$
CREATE  PROCEDURE `generate_hiv_summary_v213`()
BEGIN
                    select @_query_type := "sync"; 
					select @_start := now();
					select @_start := now();
					select @_table_version := "flat_hiv_summary_v2.13";
					...
					create table if not exists flat_hiv_summary_v213 (
						person_id int,
						uuid varchar(100),
						visit_id int,
							encounter_id int,
						...
						transfer_transfer_out_bncd int,
					...
					select @_last_update :=
						if(@_last_update is null,
							(select max(date_created) from amrs.encounter e join etl.flat_hiv_summary_v213 using (encounter_id)),
							@_last_update);
					...
					create  table if not exists flat_hiv_summary_v213_queue(person_id int, primary key (person_id));

					
					
					select @_num_ids := (select count(*) from flat_hiv_summary_v213_queue limit 1);

					if (@_num_ids=0 or @_query_type="sync") then

                        replace into flat_hiv_summary_v213_queue
					...
					select @_person_ids_count := (select count(*) from flat_hiv_summary_v213_queue);

					delete t1 from flat_hiv_summary_v213 t1 join flat_hiv_summary_v213_queue t2 using (person_id);

					while @_person_ids_count > 0 do

						
						drop table if exists flat_hiv_summary_v213_queue_0;

						create temporary table flat_hiv_summary_v213_queue_0 (select * from flat_hiv_summary_v213_queue limit 5000); 


						select @_person_ids_count := (select count(*) from flat_hiv_summary_v213_queue);

						drop table if exists flat_hiv_summary_v213_0a;
						create temporary table flat_hiv_summary_v213_0a
						(select
							t1.person_id,
							t1.visit_id,
							t1.encounter_id,
							...
							from etl.flat_obs t1
								join flat_hiv_summary_v213_queue_0 t0 using (person_id)
						...
						insert into flat_hiv_summary_v213_0a
						(select
							t1.person_id,
							null,
							t1.encounter_id,
						...
						drop table if exists flat_hiv_summary_v213_0;
						create temporary table flat_hiv_summary_v213_0(index encounter_id (encounter_id), index person_enc (person_id,encounter_datetime))
						(select * from flat_hiv_summary_v213_0a
						...
						drop temporary table if exists flat_hiv_summary_v213_1;
						create temporary table flat_hiv_summary_v213_1 (index encounter_id (encounter_id))
						(select
						...
						from flat_hiv_summary_v213_0 t1
						...
						alter table flat_hiv_summary_v213_1 drop prev_id, drop cur_id;

						drop table if exists flat_hiv_summary_v213_2;
						create temporary table flat_hiv_summary_v213_2
						(select *,
							@_prev_id := @_cur_id as prev_id,
							...
							case
							    when @_prev_id != @_cur_id then null
                                when is_clinical_encounter then @_outreach_date_bncd
                                else null
                            end as outreach_date_bncd,

                            case
                                when encounter_type=21 and @_outreach_date_bncd is null then @_outreach_date_bncd := encounter_datetime
								when is_clinical_encounter then @_outreach_date_bncd := null
								when @_prev_id != @_cur_id then @_outreach_date_bncd := null
                                else @_outreach_date_bncd
                            end as next_outreach_date_bncd,

                            case
                                when @_prev_id != @_cur_id then null
                                when is_clinical_encounter then @_outreach_death_date_bncd
                                else null
                            end as outreach_death_date_bncd,

                            case
                                when encounter_type=21 and @_outreach_death_date_bncd  is null then @_outreach_death_date_bncd  := death_date
                                when is_clinical_encounter then @_outreach_death_date_bncd  := null
                                when @_prev_id != @_cur_id then @_outreach_death_date_bncd := null
                                else @_outreach_death_date_bncd
                            end as next_outreach_death_date_bncd,


                            case
                                when @_prev_id != @_cur_id then null
                                when is_clinical_encounter then cast(@_outreach_patient_care_status_bncd as unsigned)
                                else null
                            end as outreach_patient_care_status_bncd,

                            case
                                when encounter_type=21 and @_outreach_patient_care_status_bncd is null then @_outreach_patient_care_status_bncd := patient_care_status
                                when is_clinical_encounter then @_outreach_patient_care_status_bncd := null
                                when @_prev_id != @_cur_id then @_outreach_patient_care_status_bncd := null
                                else @_outreach_patient_care_status_bncd
                            end as next_outreach_patient_care_status_bncd,

                            case
							    when @_prev_id != @_cur_id then null
                                when is_clinical_encounter then @_transfer_date_bncd
                                else null
                            end as transfer_date_bncd,

                            case
                                when encounter_type=116 and @_transfer_date_bncd is null then @_transfer_date_bncd := encounter_datetime
								when is_clinical_encounter then @_transfer_date_bncd := null
								when @_prev_id != @_cur_id then @_transfer_date_bncd := null
                                else @_transfer_date_bncd
                            end as next_transfer_date_bncd,

							case
							    when @_prev_id != @_cur_id then null
                                when is_clinical_encounter then @_transfer_transfer_out_bncd
                                else null
                            end as transfer_transfer_out_bncd,

                            case
                                when encounter_type=116 and @_transfer_transfer_out_bncd is null then @_transfer_transfer_out_bncd := encounter_datetime
								when is_clinical_encounter then @_transfer_transfer_out_bncd := null
								when @_prev_id != @_cur_id then @_transfer_transfer_out_bncd := null
                                else @_transfer_transfer_out_bncd
                            end as next_transfer_transfer_out_bncd

							from flat_hiv_summary_v213_1
							order by person_id, date(encounter_datetime) desc, encounter_type_sort_index desc
						);

						alter table flat_hiv_summary_v213_2 drop prev_id, drop cur_id, drop cur_encounter_type, drop cur_encounter_datetime, drop cur_clinical_rtc_date;
						...
						drop temporary table if exists flat_hiv_summary_v213_3;
						create temporary table flat_hiv_summary_v213_3 (prev_encounter_datetime datetime, prev_encounter_type int, index person_enc (person_id, encounter_datetime desc))
						(select
							*,
							@_prev_id := @_cur_id as prev_id,
							@_cur_id := t1.person_id as cur_id,
							...
							from flat_hiv_summary_v213_2 t1
							order by person_id, date(encounter_datetime), encounter_type_sort_index
						);

						replace into flat_hiv_summary_v213
						(select
							person_id,
							t1.uuid,
							t1.visit_id,
							...
							from flat_hiv_summary_v213_3 t1
								join amrs.location t2 using (location_id));

				    delete from flat_hiv_summary_v213_queue where person_id in (select person_id from flat_hiv_summary_v213_queue_0);

DELIMITER $$
CREATE PROCEDURE `generate_hiv_summary_v15_2`()
BEGIN
					...
					create  table if not exists flat_hiv_summary_build_queue_2(person_id int, primary key (person_id));

 

					
					
					select @num_ids := (select count(*) from flat_hiv_summary_build_queue_2 limit 1);
					...
					if (@num_ids=0 or @query_type="sync") then
                        ...
                        replace into flat_hiv_summary_build_queue_2
					...
					delete t1 from flat_hiv_summary_v15 t1 join flat_hiv_summary_build_queue_2 t2 using (person_id);

					while @person_ids_count > 0 do

						
						drop table if exists flat_hiv_summary_build_queue_2_0;

						create temporary table flat_hiv_summary_build_queue_2_0 (select * from flat_hiv_summary_build_queue_2 limit 100); 
						...
						(select
							...
							from etl.flat_obs t1
								join flat_hiv_summary_build_queue_2_0 t0 using (person_id)
				    ...
				    delete from flat_hiv_summary_build_queue_2 where person_id in (select person_id from flat_hiv_summary_build_queue_2_0);

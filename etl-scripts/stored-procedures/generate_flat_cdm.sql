DELIMITER $$
CREATE PROCEDURE `generate_flat_cdm`(IN query_type varchar(50), IN queue_number int, IN queue_size int, IN cycle_size int)
BEGIN
...
#set @query_type = "build";
					...
					set @start = now();
					set @table_version = "flat_cdm_v1.0";
					...
					#delete from etl.flat_log where table_name like "%flat_cdm%";
					#drop table etl.flat_cdm;
					create table if not exists flat_cdm (
...
#set @write_table = concat("flat_cdm_temp_",1);
#set @queue_table = concat("flat_cdm_build_queue_",1);                    												
...
#SET @dyn_sql=CONCAT('Create table if not exists ',@queue_table,' (select * from flat_cdm_build_queue limit 500);'); 
							PREPARE s1 from @dyn_sql; 
							EXECUTE s1; 
							DEALLOCATE PREPARE s1;  
							
							#delete t1 from flat_cdm_build_queue t1 join @queue_table t2 using (person_id)
...
#select max(date_created) into @last_update from etl.flat_log where table_name like "%cdm%";
...
#select @last_update;														
					...
					#delete t1 from flat_cdm t1 join @queue_table t2 using (person_id);
					...
					while @person_ids_count > 0 do
                        ...
                        #create temporary table flat_cdm_build_queue__0 (select * from flat_cdm_build_queue_2 limit 5000); #TODO - change this when data_fetch_size changes

#SET @dyn_sql=CONCAT('create temporary table flat_cdm_build_queue__0 (person_id int primary key) (select * from ',@queue_table,' limit 100);'); 
						...
						(select
							...
							case
                                when obs regexp "!!6126=" then @hb_a1c_date := ifnull(GetValues(obs_datetimes,6126),encounter_datetime)
							...
							case
                                when obs regexp "!!790=" then @creatinine_date := ifnull(GetValues(obs_datetimes,790),encounter_datetime)
                            ...
                            case
                                when obs regexp "!!1006=" then @lipid_panel_date := ifnull(GetValues(obs_datetimes,1006),encounter_datetime)

DELIMITER $$
CREATE  PROCEDURE `generate_hiv_summary_11`()
BEGIN

					select @start := now();
					select @start := now();
					select @table_version := "flat_hiv_summary_11_v2.11";
					...
					create table if not exists flat_hiv_summary_11 (
						person_id int,
						uuid varchar(100),
						visit_id int,
							encounter_id int,
            ...
            index encounter_type (encounter_type)
					...
					select @last_update :=
						if(@last_update is null,
							(select max(date_created) from amrs.encounter e join etl.flat_hiv_summary_11 using (encounter_id)),
							@last_update);
 ...
 limit 10
					...
					delete t1 from flat_hiv_summary_11 t1 join new_data_person_ids t2 using (person_id);
					...
					while @person_ids_count > 0 do
						...
						drop table if exists flat_hiv_summary_11_0a;
						create temporary table flat_hiv_summary_11_0a
						(select
							t1.person_id,
							t1.visit_id,
							t1.encounter_id,
						...
						insert into flat_hiv_summary_11_0a
						(select
							t1.person_id,
							null,
							t1.encounter_id,
						...
						drop table if exists flat_hiv_summary_11_0;
						create temporary table flat_hiv_summary_11_0(index encounter_id (encounter_id), index person_enc (person_id,encounter_datetime))
						(select * from flat_hiv_summary_11_0a
						...
						drop temporary table if exists flat_hiv_summary_11_1;
						create temporary table flat_hiv_summary_11_1 (index encounter_id (encounter_id))
						(select
						...
						alter table flat_hiv_summary_11_1 drop prev_id, drop cur_id;

						drop table if exists flat_hiv_summary_11_2;
						create temporary table flat_hiv_summary_11_2
						(select *,
							@prev_id := @cur_id as prev_id,
							...
							from flat_hiv_summary_11_1
							order by person_id, date(encounter_datetime) desc, encounter_type_sort_index desc
						);

						alter table flat_hiv_summary_11_2 drop prev_id, drop cur_id, drop cur_encounter_type, drop cur_encounter_datetime, drop cur_clinical_rtc_date;
						...
						drop temporary table if exists flat_hiv_summary_11_3;
						create temporary table flat_hiv_summary_11_3 (prev_encounter_datetime datetime, prev_encounter_type int, index person_enc (person_id, encounter_datetime desc))
						(select
							*,
							@prev_id := @cur_id as prev_id,
							@cur_id := t1.person_id as cur_id,
							...
							from flat_hiv_summary_11_2 t1
							order by person_id, date(encounter_datetime), encounter_type_sort_index
						);

						replace into flat_hiv_summary_11
						(select
							person_id,
							t1.uuid,
							t1.visit_id,

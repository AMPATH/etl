CREATE DEFINER=`amutai`@`%` PROCEDURE `etl`.`generate_flat_labs_and_imaging`(IN query_type varchar(50), IN queue_number int, IN queue_size int, IN cycle_size int)
BEGIN
        ...
        10313  DRUG SENSITIVITY TEST, IMAGE
                ...
                2322 = HPV TEST
                885 = PAP SMEAR
                12078 = CD4 COUNT RESULT QUALITATIVE 
			*/
                ...
                set @concept_ids = '(1030, 1040, 856, 5497, 730, 21,653,790,12,6126,887,6252,1537,1271,9239,9020,857
													...
													1984,2339,6337,7276,2340,9307,1327,8732,8733,8734,8735,10195,10196,10197,6342,9434,7423,2322,885,12250,12078
                )';
				...
				while @queue_count > 0 do
					...
					SET @dyn_sql= CONCAT(
									...
									(select t1.person_id,
										...
										if(obs regexp "!!6342=",cast(replace(replace((substring_index(substring(obs,locate("6342=",obs)),@sep,1)),"6342=",""),"!!","") as unsigned),null) as antibody_screen,
                                        ...
                                        if(obs regexp "!!12250=",cast(getValues(obs,12250) as decimal(5,2)),null) as egfr,
										CONCAT(
											case
                                        ...
                                        if(obs regexp "!!12078=",getValues(obs,12078),null) as cd4_lateral_flow
										from flat_lab_obs t1
											join temp_queue_table t2 using(person_id)
											left outer join flat_orders t3 using(encounter_id)
										where t1.test_datetime < t1.max_date_created
										AND NOT (obs REGEXP "!!2322=1138!!")
									)'
								);
...
*/
					...
					SET @dyn_sql = CONCAT(
						'insert ignore into ',@write_table,
						'(select
							null,
                            person_id,
							uuid,

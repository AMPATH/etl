CREATE DEFINER=`amutai`@`%` PROCEDURE `etl`.`generate_flat_lab_obs`()
BEGIN
...
#     Added Antibody test
# v2.1 
# Added HPV and PAP Smear test

select @table_version := "flat_lab_obs_v2.1";

set session group_concat_max_len=100000;
select @start := now();
select @last_date_created_enc := (select max(date_created) from amrs.encounter);
-- select @last_date_created_obs := (select max(date_created) from amrs.obs where voided=0);
select @last_date_created_obs := (select max(date_created) from amrs.encounter);
...
CREATE TABLE voided_obs ( person_id INT, obs_datetime DATETIME, INDEX person_datetime (person_id, obs_datetime) )
(select distinct person_id, obs_datetime
        from amrs.obs
        where voided=1
                and date_voided > @last_update
                and date_created <= @last_update
                and concept_id in (856,5497,730,21,653,790,12,1030,1040,12078,1271,9239,9020,9508, 6126, 887, 6252, 1537, 857,
                                ...
                                653,654,717,848,785,1014,10249,10250,10251,9010,9011,9699,9012,9812,10304,10313,8731,8595,
                                                1984,2339,6337,7276,2340,9307,1327,8732,8733,8734,8735,10195,10196,10197,6342,9434,9590,7423,885,2322,12250)
);
...
(select
        ...
        where
                concept_id in (856,5497,730,21,653,790,12,1030,1040,12078,1271,9239,9020,9508, 6126, 887, 6252, 1537, 857,
                        679,21,851,1018,1017,1016,729,678,1330,790,1132,1133,1134,655,1297,6123,
                                                653,654,717,848,785,1014,10249,10250,10251,9010,9011,9699,9012,9812,10304,10313,8731,8595,
                                                1984,2339,6337,7276,2340,9307,1327,8732,8733,8734,8735,10195,10196,10197,6342,9434,9590,7423,885,2322,12250)

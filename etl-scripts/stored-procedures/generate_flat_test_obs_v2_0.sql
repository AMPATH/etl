DELIMITER $$
CREATE PROCEDURE `generate_flat_test_obs_v2_0`(IN query_type varchar(50), IN queue_number int, IN queue_size int, IN cycle_size int)
BEGIN

				set session group_concat_max_len=100000;
				set @start = now();
				set @primary_table := "flat_test_obs";
				set @table_version = "flat_test_obs_v2.0";
...
#set @cycle_size = 1000;
				...
				SET @dyn_sql = CONCAT('create table if not exists ', @primary_table,
						'(date_created TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,						
							...
							primary key (person_id, test_datetime)
						);');
		...
		select CONCAT('BUILDING ',@primary_table,'..........................................') as 'Job';
						...
						SET @dyn_sql=CONCAT(
							...
							(select * from ',@build_queue,' limit ', @queue_size, ');'
                            ); 
						PREPARE s1 from @dyn_sql; 
						EXECUTE s1; 
						DEALLOCATE PREPARE s1;  
						...
						SET @dyn_sql=CONCAT('delete t1 from ',@build_queue, ' t1 join ',@queue_table, ' t2 using (person_id, obs_datetime);'); 
						PREPARE s1 from @dyn_sql; 
						EXECUTE s1; 
						DEALLOCATE PREPARE s1;  
		...
		select CONCAT('SYNCING ',@primary_table,'..........................................');
						...
						create table if not exists flat_test_obs_sync_queue (person_id int, obs_datetime datetime, index test_datetime (person_id, obs_datetime));                            
...
#set @last_update = "2018-01-26";						
                      ...
                      SET @dyn_sql=CONCAT('replace into ',@queue_table,
								'(select person_id, obs_datetime
                                    ...
                                    where 
										t1.date_created >= @last_update and t1.voided=0 
                                       AND 
											CASE

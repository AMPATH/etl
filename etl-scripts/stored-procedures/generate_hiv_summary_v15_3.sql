DELIMITER $$
CREATE PROCEDURE `generate_hiv_summary_v15_3`(IN query_type varchar(50), IN queue_number int, IN queue_size int, IN cycle_size int)
BEGIN
					...
					#delete from etl.flat_log where table_name like "%15%";
					#drop table etl.flat_hiv_summary_v15b;
					create table if not exists flat_hiv_summary_v15b (
                        ...
                        outreach_date_bncd datetime, #bncd means before next clinical datetime                        
...
#					select @person_ids_count := (select count(*) from flat_hiv_summary_build_queue_2);

					#delete t1 from flat_hiv_summary_v15b t1 join @queue_table t2 using (person_id);
					...
					while @person_ids_count > 0 do
                        ...
                        #create temporary table flat_hiv_summary_build_queue__0 (select * from flat_hiv_summary_build_queue_2 limit 5000); #TODO - change this when data_fetch_size changes
						...
						(select
						    ...
						    case
                                ...
                                when t1.encounter_type in (129) then 5 #MDT ahead of other clinical encounters
								else 1
						...
						(select
							...
							case
								...
								when obs regexp "!!1250=" then @cur_arv_meds := normalize_arvs(obs,'1250')
									#@cur_arv_meds := getArvConceptIds(obs,1250)
								...
								when obs regexp "!!1088=" then @cur_arv_meds := normalize_arvs(obs,'1088')
									#@cur_arv_meds := getArvConceptIds(obs,1088)
								...
								when obs regexp "!!2154=" then @cur_arv_meds := normalize_arvs(obs,'2154')
									#@cur_arv_meds :=getArvConceptIds(obs,2154)
							...
							case
								...
								when obs regexp "!!1250=" then @cur_arv_meds := normalize_arvs(obs,'1250')
									#getArvConceptIds(obs,1250)
									#replace(replace((substring_index(substring(obs,locate("!!1250=",obs)),@sep,ROUND ((LENGTH(obs) - LENGTH( REPLACE ( obs, "!!1250=", "") ) ) / LENGTH("!!1250=") ))),"!!1250=",""),"!!","")
								when obs regexp "!!1088=" then @cur_arv_meds := normalize_arvs(obs,'1088')
									#getArvConceptIds(obs,1088)
									#replace(replace((substring_index(substring(obs,locate("!!1088=",obs)),@sep,ROUND ((LENGTH(obs) - LENGTH( REPLACE ( obs, "!!1088=", "") ) ) / LENGTH("!!1088=") ))),"!!1088=",""),"!!","")
								when obs regexp "!!2154=" then @cur_arv_meds := normalize_arvs(obs,'2154')
									#getArvConceptIds(obs,2154)
									#replace(replace((substring_index(substring(obs,locate("!!2154=",obs)),@sep,ROUND ((LENGTH(obs) - LENGTH( REPLACE ( obs, "!!2154=", "") ) ) / LENGTH("!!2154=") ))),"!!2154=",""),"!!","")
								else null
							...
							# 1499 = ANTIRETROVIRAL DRUG TREATMENT START DATE
							case
								...
								when obs regexp "!!7015=" and @arv_first_regimen_start_date is null then @arv_first_regimen_start_date := "1900-01-01" 
                                ...
                                when @prev_id != @cur_id and @cur_arv_meds is not null then @arv_first_regimen_start_date := date(t1.encounter_datetime)
                            ...
                            # ADD strict defintion for arv_first_regimen_start_date,
                            # Make sure the start arv question is required
                            
                            case
								when (@prev_id != @cur_id or @arv_first_regimen is null) and obs regexp "!!1499=" then  @arv_first_regimen_location_id := 9999
								when (@prev_id != @cur_id or @arv_first_regimen_location_id is null) and @arv_first_regimen = "unknown" then @arv_first_regimen_location_id := 9999
								when (@prev_id != @cur_id or @arv_first_regimen_location_id is null) and @arv_first_regimen != "unknown" then @arv_first_regimen_location_id := location_id
								when(@prev_id != @cur_id or @arv_first_regimen_location_id is null) and @arv_first_regimen is not null then @arv_first_regimen_location_id := location_id
							...
							# 6976 HIV ANTIRETROVIRAL DRUG PLAN TREATMENT CATEGORY
							# 6744 CURRENT HIV ANTIRETROVIRAL DRUG USE TREATMENT CATEGORY
                            # 6695 THIRD LINE HIV ANTIRETROVIRAL DRUG TREATMENT
							# 6693 FIRST LINE HIV ANTIRETROVIRAL DRUG TREATMENT
							# 6694 SECOND LINE HIV ANTIRETROVIRAL DRUG TREATMENT 
                            # UNKNOWN ANTIRETROVIRAL DRUG (5811)
                            ...
                            # 8351 = 	PREGNANCY STATUS, CODED
                            # PREGNANCY, MISCARRIAGE (48)
							...
							# 1850 = RESTART
							case
								when obs regexp "!!6174=" then @tb_screen := true #there is an obs for "any symptoms of tb?"
								when obs regexp "!!2022=1065!!" then @tb_screen := true #1065 = yes
								when obs regexp "!!307=" then @tb_screen := true #test result for tb
								when obs regexp "!!12=" then @tb_screen := true #test result for tb
								when obs regexp "!!1271=(12|307|8064|2311|2323)!!" then @tb_screen := true #test ordered for tb
								when orders regexp "(12|307|8064|2311|2323)" then @tb_screen := true #test ordered for tb
								when obs regexp "!!1866=(12|307|8064|2311|2323)!!" then @tb_screen := true #test results for tb
								when obs regexp "!!5958=1077!!" then @tb_screen := true #means cough is bloody
								when obs regexp "!!2020=1065!!" then @tb_screen := true #means a familiy member was diagnosed for tb
								when obs regexp "!!2021=1065!!" then @tb_screen := true #means a familiy member was diagnosed for tb
								when obs regexp "!!2028=" then @tb_screen := true #TB DIAGNOSIS MADE ON THE BASIS OF
								when obs regexp "!!1268=(1256|1850)!!" then @tb_screen := true
								when obs regexp "!!5959=(1073|1074)!!" then @tb_screen := true #COUGH DURATION, CODED
								when obs regexp "!!5971=(1073|1074)!!" then @tb_screen := true #CHEST PAIN DURATION, CODED
								when obs regexp "!!1492=107!!" then @tb_screen := true #REVIEW OF SYSTEM, EXPRESS CARE(1492)=COUGH (107)
                            ...
                            case
                                ...
                                else @tb_screening_result := null
                            end as tb_screening_result,

							case
								when obs regexp "!!6174=" then @tb_screening_datetime := encounter_datetime #there is an obs for "any symptoms of tb?"
								when obs regexp "!!2022=1065!!" then @tb_screening_datetime := encounter_datetime #1065 = yes
								when obs regexp "!!307=" then @tb_screening_datetime := encounter_datetime #test result for tb
								when obs regexp "!!12=" then @tb_screening_datetime := encounter_datetime #test result for tb
								when obs regexp "!!1271=(12|307|8064|2311|2323)!!" then @tb_screening_datetime := encounter_datetime #test ordered for tb
								when orders regexp "(12|307|8064|2311|2323)" then @tb_screening_datetime := encounter_datetime #test ordered for tb
								when obs regexp "!!1866=(12|307|8064|2311|2323)!!" then @tb_screening_datetime := encounter_datetime #test results for tb
								when obs regexp "!!5958=1077!!" then @tb_screening_datetime := encounter_datetime #means cough is bloody
								when obs regexp "!!2020=1065!!" then @tb_screening_datetime := encounter_datetime #means a familiy member was diagnosed for tb
								when obs regexp "!!2021=1065!!" then @tb_screening_datetime := encounter_datetime #means a familiy member was diagnosed for tb
								when obs regexp "!!2028=" then @tb_screening_datetime := encounter_datetime #TB DIAGNOSIS MADE ON THE BASIS OF
								when obs regexp "!!1268=(1256|1850)!!" then @tb_screening_datetime := encounter_datetime
								when obs regexp "!!5959=(1073|1074)!!" then @tb_screening_datetime := encounter_datetime #COUGH DURATION, CODED
								when obs regexp "!!5971=(1073|1074)!!" then @tb_screening_datetime := encounter_datetime #CHEST PAIN DURATION, CODED
								when obs regexp "!!1492=107!!" then @tb_screening_datetime := encounter_datetime #REVIEW OF SYSTEM, EXPRESS CARE(1492)=COUGH (107)
							...
							case
                                ...
                                when obs regexp "!!1268=" then @on_tb_tx := 1 #first statment above catches all negatives, this statement catches all codes where patient getting drugs
                            ...
                            # 780 = ORAL CONTRACEPTION
                            # 5275 = INTRAUTERINE DEVICE
                            # 5279 = 	INJECTABLE CONTRACEPTIVES
                            # 6220 = CONTRACEPTIVE IMPLANT
							# 7240 = FAMILY PLANNING METHOD CHOSEN

							case
								when obs regexp "!!7240=1107!!" then @modern_contraceptive_method_start_date := null
						...
						(select
							*,
							...
							# 9082 = PATIENT CARE STATUS
							# 1286 = AMPATH (1286)
							# 1287 = NON-AMPATH (1287)
							# 9068 = AMPATH, NON-AMRS (9068)
							# 2050 = MATERNAL CHILD HEALTH PROGRAM (2050)							
				    ...
				    #delete from @queue_table where person_id in (select person_id from flat_hiv_summary_build_queue__0);

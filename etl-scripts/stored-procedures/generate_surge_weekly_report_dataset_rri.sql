DELIMITER $$
CREATE PROCEDURE `generate_surge_weekly_report_dataset_rri`(IN query_type varchar(50), IN queue_number int, IN queue_size int, IN cycle_size int , IN log boolean)
BEGIN
            set @primary_table := "surge_weekly_report_dataset_rri";
            set @query_type = query_type;
            set @queue_table = "";

            set @total_rows_written = 0;
  			set @start = now();
  			set @table_version = "surge_weekly_report_dataset_v13_rri";
...
CREATE TABLE IF NOT EXISTS surge_weekly_report_dataset_rri(
...
while @person_ids_count > 0 do
            ...
            (SELECT
				...
				TIMESTAMPDIFF(DAY,
                    prev_rtc_date,
                    end_date) AS days_since_prev_rtc_date,
                ...
                case 
                	when t2.visit_type is not null and t2.visit_type in (23, 119, 124) then @location_id := t2.prev_clinical_location_id
                    else @location_id := t2.location_id
                end as location_id,
                ...
                IF(YEARWEEK(arv_first_regimen_start_date) = YEARWEEK(prev_clinical_datetime_hiv)
                    ...
                    null as  breast_feeding,
                    null as breast_feeding_encounter_date,
			...
			`days_since_prev_rtc_date` bigint(21) DEFAULT NULL,
            ...
            CREATE temporary TABLE surge_weekly_report_dataset__2 (SELECT *,
                ...
                CASE
                    WHEN
                        (@prev_id != @cur_id
                            OR @ever_active_after_sep_20 IS NULL)
                            AND year_week >=202039
                            AND (@cur_status = 'active' OR @cur_status = 'missed' OR @cur_status = 'defaulter')
                            AND ((death_date IS NULL OR DATE(death_reporting_date) > '2020-09-26') AND fd_next_encounter_datetime IS NULL)
                            AND (transfer_out_date IS NULL OR (DATE(transfer_reporting_date) > '2020-09-26'))
                            AND ((exit_reporting_date IS NULL OR DATE(exit_reporting_date) > '2020-09-26') AND fe_next_encounter_datetime IS NULL)
                            
                    THEN
                        @ever_active_after_sep_20:=1
                    WHEN @prev_id != @cur_id THEN @ever_active_after_sep_20:=NULL
                    ELSE @ever_active_after_sep_20
                END AS ever_active_after_sep_20,  #active_after_may_19

                CASE
                    WHEN
                        (@prev_id != @cur_id
                            OR @is_ltfu_after_sep_20 IS NULL) 
                            AND @ever_active_after_sep_20 = 1
                            AND @cur_status = 'ltfu'
                            AND year_week > 202039
                    THEN
                        @is_ltfu_after_sep_20:=1
                    WHEN
                        (@prev_id != @cur_id
                            OR @is_ltfu_after_sep_20 IS NULL)
                            AND NOT (@ever_active_after_sep_20 = NULL)
                            AND year_week > 202039
                            
                    THEN
                        @is_ltfu_after_sep_20:=0
					when @prev_id = @cur_id and @is_ltfu_after_sep_20 = 1 then @is_ltfu_after_sep_20 := 1
					when @prev_id = @cur_id AND year_week > 202039  and (@prev_status = 'defaulter' AND (days_since_rtc_date > 28 OR (days_diff_enc_date_and_prev_rtc > 28 and days_since_rtc_date <=0) 
							OR (days_since_prev_rtc_date > 28 and days_since_rtc_date <=0))) then @is_ltfu_after_sep_20 := 1
                    WHEN @prev_id != @cur_id THEN @is_ltfu_after_sep_20:=NULL
                    ELSE @is_ltfu_after_sep_20
                END AS is_ltfu_after_sep_20,   #is_ltfu_after_may_revised
				
                #IF(@is_ltfu_after_sep_20 = 1,1,0) AS is_ltfu_after_sep_total,
                IF((@is_ltfu_after_sep_20 = 1 OR 
                (@prev_status = 'defaulter' AND year_week > 202039 AND ((days_since_rtc_date > 28) OR (days_diff_enc_date_and_prev_rtc > 28 and days_since_rtc_date <=0)))),
                    1,
                    0) AS is_ltfu_after_sep_total,

                IF(@is_ltfu_after_sep_20 = 1
                AND @cur_status = 'ltfu'
                ...
                IF((@cur_status = 'ltfu' OR days_since_rtc_date > 30)
                    AND ((death_date IS NULL OR DATE(death_reporting_date) > end_date) AND fd_next_encounter_datetime IS NULL)
                    AND (transfer_out_date IS NULL OR (DATE(transfer_reporting_date) > end_date))
                    AND ((exit_reporting_date IS NULL OR DATE(exit_reporting_date) > end_date) AND fe_next_encounter_datetime IS NULL),
                    1,
                ...
                IF(((@is_ltfu_after_sep_20 = 1 AND @prev_status = 'ltfu')
                        OR (@prev_status = 'defaulter' AND defaulted = 0 AND ((days_since_rtc_date > 28) OR (days_diff_enc_date_and_prev_rtc > 28 and days_since_rtc_date <= 0 ) OR (days_since_prev_rtc_date > 28 and days_since_rtc_date <=0)))) 
                            AND @cur_status = 'dead',
                    1,
			...
			case when (cur_program = 3 or cur_program = 9) AND (@cur_status = 'active' or @cur_status = 'missed' or @cur_status = 'defaulter') then @active_on_dc := 1
            else @active_on_dc := 0
            end as active_on_dc,
            
            case when (cur_program = 3 or cur_program = 9) AND (@cur_status = 'active' or @cur_status = 'missed' or @cur_status = 'defaulter') then @on_dc_this_week := 1
            else @on_dc_this_week := 0
            end as on_dc_this_week,
            
			#IF((t4.program_id = 3 OR t4.program_id = 9) AND (@status = 'active' or @status = 'missed' or @status = 'defaulter'), 1, 0) AS active_on_dc,
			#IF((t4.program_id = 3 OR t4.program_id = 9) AND (@status = 'active' or @status = 'missed' or @status = 'defaulter') ,1,0) AS on_dc_this_week,
			 CASE  
            ...
            replace into surge_weekly_report_dataset_rri						  
                        (select
                        now(),
                        elastic_id ,
                        person_uuid ,
					...
					SELECT CONCAT('select from surge_weekly_report_dataset_sync_queue ...');
          			select count(*) from etl.surge_weekly_report_dataset_sync_queue;

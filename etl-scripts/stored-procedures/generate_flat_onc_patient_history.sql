...
BEGIN
					...
					CREATE TABLE IF NOT EXISTS flat_onc_patient_history (
						...
						cur_onc_meds INT,
						cur_onc_meds_dose INT,
						cur_onc_meds_frequency INT,
					...
					while @person_ids_count > 0 do
						...
						SET @dyn_sql = CONCAT(
								...
								from etl.flat_obs t1
									...
									join etl.flat_orders t2 using(encounter_id)
						...
						SELECT @rtc_date:=NULL;
						SELECT @diagnosis:=NULL;
						SELECT @diagnosis_method:=NULL;
						SELECT @diagnosis_date:=NULL;
						SELECT @cancer_type:=NULL;
						SELECT @breast_cancer_type:=NULL;
						SELECT @non_cancer_diagnosis:=NULL;
						SELECT @cancer_stage:=NULL;
						SELECT @overal_cancer_stage_group:=NULL;
						SELECT @cur_onc_meds_start_date:=NULL;
						SELECT @cur_onc_meds_end_date:=NULL;
						SELECT @cur_onc_meds:=NULL;
						SELECT @cur_onc_meds_dose:=NULL;
						SELECT @cur_onc_meds_frequency:=NULL;
						SELECT @oncology_treatment_plan:=NULL;
						SELECT @reasons_for_surgery:=NULL;
						SELECT @chemotherapy_intent:=NULL;
						SELECT @chemotherapy_plan:=NULL;
						SELECT @drug_route:=NULL;
						SELECT @medication_history:=NULL;
						SELECT @other_meds_added:=NULL;
						SELECT @sickle_cell_drugs:=NULL;
						...
						(select 
                            ...
                            @cancer_type := GetValues(obs, 7176) as cancer_type,
                            @breast_cancer_type := GetValues(obs, 9841) as breast_cancer_type,
                            @non_cancer_diagnosis := GetValues(obs, 9847) as non_cancer_diagnosis,
                            @cancer_stage := GetValues(obs, 6582) as cancer_stage,
                            @overal_cancer_stage_group := GetValues(obs, 9868) as overal_cancer_stage_group,
                            case
                            ...
                            @cur_onc_meds_dose := GetValues(obs, 1899) as cur_onc_meds_dose,
                            @cur_onc_meds_frequency := GetValues(obs, 1896) as cur_onc_meds_frequency,
                            case
                            ...
                            @oncology_treatment_plan := GetValues(obs, 8723) as oncology_treatment_plan,
                            @reasons_for_surgery := GetValues(obs, 8725) as reasons_for_surgery,
                            @chemotherapy_intent := GetValues(obs, 2206) as chemotherapy_intent,
                            @chemotherapy_plan := GetValues(obs, 9869) as chemotherapy_plan,
                            @drug_route := GetValues(obs, 7463) as drug_route,
                            @medication_history := GetValues(obs, 9918) as medication_history,
                            @other_meds_added := GetValues(obs, 10198) as other_meds_added,
                            @sickle_cell_drugs := GetValues(obs, 9710) as sickle_cell_drugs
                            
						from flat_onc_patient_history_0 t1
							join amrs.person p using (person_id)
                            join amrs.visit v using (visit_type_id)
                            join etl.clinical_encounter_type_map c using(encounter_type)

DELIMITER $$
CREATE PROCEDURE `generate_flat_nutrition`()
BEGIN
...
create table if not exists flat_nutrition (
    ...
    bmi varchar(100),
    bmi_for_age_category_coded varchar(200),
...
#insert into new_data_person_ids VALUES (904993);


drop table if exists flat_nutrition_0;
create table flat_nutrition_0(encounter_id int, primary key (encounter_id), index person_enc_date (person_id,encounter_datetime))
(select
	t1.person_id,
	t1.encounter_id,
...
drop  table if exists flat_nutrition_1;
create  table flat_nutrition_1 (index encounter_id (encounter_id))
(select

DELIMITER $$
CREATE  PROCEDURE `generate_hiv_smry_next`()
BEGIN
                    select @query_type := "sync"; 
					select @start := now();
					select @start := now();
					...
					create table if not exists flat_hiv_smry_next (
						person_id int,
						uuid varchar(100),
						visit_id int,
							encounter_id int,
					...
					select @last_update :=
						if(@last_update is null,
							(select max(date_created) from amrs.encounter e join etl.flat_hiv_smry_next using (encounter_id)),
							@last_update);
					...
					create  table if not exists flat_hiv_smry_next_queue(person_id int, primary key (person_id));

					
					
					select @num_ids := (select count(*) from flat_hiv_smry_next_queue limit 1);

					if (@num_ids=0 or @query_type="sync") then

                        replace into flat_hiv_smry_next_queue
					...
					select @person_ids_count := (select count(*) from flat_hiv_smry_next_queue);

					delete t1 from flat_hiv_smry_next t1 join flat_hiv_smry_next_queue t2 using (person_id);

					while @person_ids_count > 0 do

						
						drop table if exists flat_hiv_smry_next_queue_0;

						create temporary table flat_hiv_smry_next_queue_0 (select * from flat_hiv_smry_next_queue limit 5000); 


						select @person_ids_count := (select count(*) from flat_hiv_smry_next_queue);

						drop table if exists flat_hiv_smry_next_0a;
						create temporary table flat_hiv_smry_next_0a
						(select
							t1.person_id,
							t1.visit_id,
							t1.encounter_id,
							...
							from etl.flat_obs t1
								join flat_hiv_smry_next_queue_0 t0 using (person_id)
						...
						insert into flat_hiv_smry_next_0a
						(select
							t1.person_id,
							null,
							t1.encounter_id,
						...
						drop table if exists flat_hiv_smry_next_0;
						create temporary table flat_hiv_smry_next_0(index encounter_id (encounter_id), index person_enc (person_id,encounter_datetime))
						(select * from flat_hiv_smry_next_0a
						...
						drop temporary table if exists flat_hiv_smry_next_1;
						create temporary table flat_hiv_smry_next_1 (index encounter_id (encounter_id))
						(select
				        ...
				        when @prev_id=@cur_id then @prev_arv_start_date := @arv_start_date
							...
							case
								...
								when obs regexp "!!2022=1065!!" then @tb_screening_result := true 
								when obs regexp "!!307=" then @tb_screening_result := true 
								when obs regexp "!!12=" then @tb_screening_result := true 
								when obs regexp "!!1271=(12|307|8064|2311|2323)!!" then @tb_screening_result := true 
								when orders regexp "(12|307|8064|2311|2323)" then @tb_screening_result := true 
								when obs regexp "!!1866=(12|307|8064|2311|2323)!!" then @tb_screening_result := true 
								when obs regexp "!!5958=1077!!" then @tb_screening_result := true 
								when obs regexp "!!2020=1065!!" then @tb_screening_result := true 
								when obs regexp "!!2021=1065!!" then @tb_screening_result := true 
								when obs regexp "!!2028=" then @tb_screening_result := true 
								when obs regexp "!!1268=(1256|1850)!!" then @tb_screening_result := true
								when obs regexp "!!5959=(1073|1074)!!" then @tb_screening_result := true 
								when obs regexp "!!5971=(1073|1074)!!" then @tb_screening_result := true 
								when obs regexp "!!1492=107!!" then @tb_screening_result := true 
						...
						alter table flat_hiv_smry_next_1 drop prev_id, drop cur_id;

						drop table if exists flat_hiv_smry_next_2;
						create temporary table flat_hiv_smry_next_2
						(select *,
							@prev_id := @cur_id as prev_id,
							...
							from flat_hiv_smry_next_1
							order by person_id, date(encounter_datetime) desc, encounter_type_sort_index desc
						);

						alter table flat_hiv_smry_next_2 drop prev_id, drop cur_id, drop cur_encounter_type, drop cur_encounter_datetime, drop cur_clinical_rtc_date;
						...
						drop temporary table if exists flat_hiv_smry_next_3;
						create temporary table flat_hiv_smry_next_3 (prev_encounter_datetime datetime, prev_encounter_type int, index person_enc (person_id, encounter_datetime desc))
						(select
							*,
							@prev_id := @cur_id as prev_id,
							@cur_id := t1.person_id as cur_id,
							...
							from flat_hiv_smry_next_2 t1
							order by person_id, date(encounter_datetime), encounter_type_sort_index
						);

						replace into flat_hiv_smry_next
						(select
							person_id,
							t1.uuid,
							t1.visit_id,
							...
							from flat_hiv_smry_next_3 t1
								join amrs.location t2 using (location_id));

				    delete from flat_hiv_smry_next_queue where person_id in (select person_id from flat_hiv_smry_next_queue_0);

				 end while;

				
				delete t1
				from flat_hiv_smry_next t1

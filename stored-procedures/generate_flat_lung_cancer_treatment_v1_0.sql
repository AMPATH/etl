CREATE PROCEDURE `generate_flat_lung_cancer_treatment_v1_0`(IN query_type varchar(50), IN queue_number int, IN queue_size int, IN cycle_size int)
BEGIN
...
#set @write_table = concat("flat_lung_cancer_treatment_temp_",1);
#set @queue_table = concat("flat_lung_cancer_treatment_build_queue_",1);                    												
...
#drop table if exists flat_lung_cancer_treatment_temp_1;							
...
#SET @dyn_sql=CONCAT('Create table if not exists ',@queue_table,' (select * from flat_lung_cancer_treatment_build_queue limit 500);'); 
							PREPARE s1 from @dyn_sql; 
							EXECUTE s1; 
							DEALLOCATE PREPARE s1;  
							
							#delete t1 from flat_lung_cancer_treatment_build_queue t1 join @queue_table t2 using (person_id)
...
SELECT 'Finding patients in flat_orders...';
					...
					#delete t1 from flat_lung_cancer_treatment t1 join @queue_table t2 using (person_id);
					...
					while @person_ids_count > 0 do
                        ...
                        #create temporary table flat_lung_cancer_treatment_build_queue__0 (select * from flat_lung_cancer_treatment_build_queue_2 limit 5000); #TODO - change this when data_fetch_size changes

#SET @dyn_sql=CONCAT('create temporary table flat_lung_cancer_treatment_build_queue__0 (person_id int primary key) (select * from ',@queue_table,' limit 100);'); 
						...
						set @disease_diagnosis_year = null;
						set @hiv_status = null;
                    ...
                    case 
					  when obs regexp "!!9281=" then @disease_diagnosis_year = GetValues(obs,9281)
					  else @disease_diagnosis_year = null
					end as disease_diagnosis_year,
					case	
...
SELECT @total_rows_written;
					...
					#select @person_ids_count := (select count(*) from flat_lung_cancer_treatment_build_queue_2);                        
					SET @dyn_sql=CONCAT('select count(*) into @person_ids_count from ',@queue_table,';'); 
					PREPARE s1 from @dyn_sql; 
					EXECUTE s1; 
					DEALLOCATE PREPARE s1;  

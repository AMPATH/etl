...
#select @last_enc_date_created := '2014-04-20';
...
#select @last_obs_date_created := '2014-04-20';
...
(select 
    ...
    concept_id,
    date_created 
from amrs.obs use index (date_created)
where voided = 1
	and date_voided > @last_obs_date_created
	and date_created <= @last_obs_date_created
	and concept_id in (1042 , 1040, 856, 5497, 730,1030));
...
where o.date_created > @last_obs_date_created and o.voided=0 and e.date_created <= @last_enc_date_created and e.voided=0 and concept_id in (1042,1040,856,5497,730,1030) 
);
...
(select * from amrs.obs o where concept_id in (1042,1040,856,5497,730,1030) and o.voided=0 and date_created > @last_obs_date_created);
...
(select t1.* 
	...
	where t1.voided=0 and t1.date_created <= @last_obs_date_created and t2.encounter_id is not null and t1.concept_id in (1042,1040,856,5497,730,1030)
);
...
(select o.*
	from amrs.encounter e
	join amrs.obs o using (encounter_id)
	where o.date_created > @last_obs_date_created and o.voided=0 and e.date_created <= @last_enc_date_created and e.voided=0 and o.concept_id in (1042,1040,856,5497,730,1030)
);
...
(select 
	...
	max(if(concept_id=1030,value_coded,null)) as enc_hiv_dna_pcr,
...
(select 
	...
	if(concept_id=1030,value_coded,null) as lab_hiv_dna_pcr,
...
insert ignore into new_person_data
(select person_id from amrs.obs where date_created <= @last_obs_date_created and date_voided > @last_obs_date_created and concept_id in (1042,1040,856,5497,730,1030));
...
#if(lab_dna_hiv_pcr,lab_dna_hiv_pcr,if(enc_dna_hiv_pcr,enc_dna_hiv_pcr,null)) as hiv_dna_pcr,

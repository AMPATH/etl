...
select @last_update :=
	if(@last_update is null, 
		(select max(date_created) from amrs.encounter e join flat_tb using (encounter_id)),
		@last_update);
...
#delete from flat_log where table_name="flat_tb";
#delete from flat_tb;
...
(select * from amrs.obs o use index (encounter_date_created) where concept_id in (2021,2022,1110,1111,1264,1265,1266,1268,1269,1270,1506,2028,5965,6077,6174,6206,1113) and o.voided=0 and date_created > @last_update);
...
insert ignore into obs_subset
(select o.* from amrs.encounter e join amrs.obs o use index (encounter_date_created) using (encounter_id) where o.date_created > @last_update and o.voided=0 and e.date_created <= @last_update and e.voided=0 and o.concept_id in (2021,2022,1110,1111,1264,1265,1266,1268,1269,1270,1506,2028,5965,6077,6174,6206,1113));

...
create temporary table n_obs (encounter_id int, encounter_datetime datetime, rtc_date datetime, on_lopinavir tinyint, on_nnrti tinyint, transfer_out tinyint, transfer_in tinyint, outreach_found_patient tinyint, reason_for_missed_appt varchar(300), index encounter_id (encounter_id))
(select person_id, 
obs_datetime as encounter_datetime, 
encounter_id,
...
max(if(concept_id in (1042,1040) and value_coded=703,1,0)) as tested_hiv_positive,
...
max(if(concept_id=856, value_numeric,null)) as vl,
max(if(concept_id=5497,value_numeric,null)) as cd4_count, 
max(if(concept_id=730,value_numeric,null)) as cd4_percent,
min(case
...
from amrs.obs o
where concept_id in (1088,1941,5096,1777,1285,1596,6206,1733,1042,1559,1725,1040,1579,1499,1255,856,5497,730,1250) and o.voided=0
...
(select *
from n_obs n1
left outer join enc e1 using (person_id,encounter_datetime,encounter_id)
left outer join address a using (person_id)
left outer join death_dates d using (person_id)
order by n1.person_id, n1.encounter_datetime);


#update ltfu_1 set encounter_type = if(encounter_type,encounter_type,999);
drop temporary table enc;
drop temporary table address;
drop temporary table n_obs;
drop temporary table death_dates;
...
alter table ltfu_2a drop prev_id, drop cur_id, drop vl, drop cd4_count, drop cd4_percent;
delete from ltfu_2a where encounter_id > 10000000 or encounter_type in (5,6,7,8,9,45,87,999) or encounter_type is null;
...
create table lost_to_follow_up (id MEDIUMINT NOT NULL AUTO_INCREMENT, primary key(id), index person_id (person_id), index encounter_datetime (encounter_datetime))
(select * from ltfu_3 order by person_id, encounter_datetime, encounter_id);

drop temporary table if exists ltfu_3;

# select person_id, prev_appt_date, encounter_datetime from ltfu_2;
# select encounter_id,person_id, prev_appt_date, encounter_datetime, next_appt_date, prev_encounter_type, encounter_type, next_encounter_type, plan, on_nnrti, on_lopinavir from lost_to_follow_up order by person_id, encounter_datetime
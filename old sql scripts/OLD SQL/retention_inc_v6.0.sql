...
# Build LTFU table
#drop table flat_retention_defined;


create table if not exists flat_retention_defined
(person_id int,encounter_id int,encounter_datetime datetime,provider_id int,location_id mediumint,encounter_type mediumint,enc_date_created datetime, transfer_care int,discontinue_is_hiv_neg int,rtc_date datetime,arv_meds varchar(500),arv_plan int,arv_meds_plan int,ampath_status int,obs_date_created datetime,gender varchar(10),birth_date datetime,person_name varchar(500),phone_number varchar(100),identifier varchar(50), index encounter_id (encounter_id), index person_enc (person_id,encounter_datetime), index enc_date_created (enc_date_created), index obs_date_created (obs_date_created));
...
where e.date_created <= @last_enc_date_created and e.voided=0 and v.encounter_id is not null and e.encounter_type not in (5,6,7,8,9,45,87)
);
...
where o.date_created > @last_obs_date_created and o.voided=0 and e.date_created <= @last_enc_date_created and e.voided=0 and concept_id in (1285,1946,5096,1502,1777,1088,1255,1250,9082)  and encounter_type not in (5,6,7,8,9,45,87)
);
...
(select 
	...
	join enc n on n.person_id = i.patient_id
	where voided=0
...
#********************************************************************************************************
#rebuild any patients that have a voided obs. 
...
(select t1.*,
	if(t3.lab_hiv_test,t3.lab_hiv_test,t3.enc_hiv_test) as hiv_test, t3.cd4_1, t3.cd4_1_date,
...
alter table flat_retention_derived_2 drop prev_id, drop cur_id, drop cur_encounter_type, drop cur_appt_date;
...
create temporary table flat_retention_derived_3 (encounter_datetime datetime, person_id int, prev_appt_date datetime, cur_appt_date datetime, prev_rtc_date datetime, cur_rtc_date datetime, prev_encounter_type tinyint, arv_start_date datetime, prev_arv_start_date datetime,hiv_start_date datetime, index person_enc (person_id, encounter_datetime desc), index encounter_id (encounter_id))
(select *,
@prev_id := @cur_id as prev_id, 
@cur_id := person_id as cur_id,
...
(encounter_id int,person_id int,prev_encounter_type int,prev_rtc_date datetime,prev_appt_date datetime,hiv_start_date datetime,arv_start_date datetime,next_encounter_type int,next_appt_date datetime, index encounter_id (encounter_id), index person_id (person_id));
...
(select encounter_id,person_id,prev_encounter_type,prev_rtc_date,prev_appt_date,hiv_start_date,arv_start_date,next_encounter_type,next_appt_date

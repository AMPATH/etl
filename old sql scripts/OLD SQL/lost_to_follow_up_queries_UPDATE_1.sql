...
create temporary table voided_obs
(select person_id, encounter_id, obs_id, obs_datetime, date_voided 
...
# build a dataset of all NEW relevant encounter data 
...
# add to the enc dataset all existing encounters with voided obs as these rows were deleted from the denormalized table. 
insert into enc
(select e.encounter_id, e.encounter_datetime, e.provider_id, e.patient_id as person_id, e.location_id,e.encounter_type, e.date_created as enc_date_created
from amrs.encounter e
...
/*
# add to the enc dataset all existing pseudo lab encounters which have voided labs
insert into enc
(select e.encounter_id, e.encounter_datetime, e.provider_id, e.patient_id as person_id, e.location_id,e.encounter_type, e.date_created as enc_date_created
from amrs.encounter e
...
# add obs from encounters with voided obs. 
insert into obs_dataset
...
select @index := max(encounter_id) + 10000000 from amrs.ltfu_1;

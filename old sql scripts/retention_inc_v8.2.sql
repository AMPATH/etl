...
(person_id int, encounter_id int,encounter_datetime datetime,provider_id int,location_id mediumint,encounter_type mediumint,enc_date_created datetime, transfer_care int,reason_exited_care int, discontinue_is_hiv_neg int,rtc_date datetime,arv_meds varchar(500),arv_plan int,arv_meds_plan varchar(500),ampath_status int,obs_date_created datetime,uuid varchar(500), gender varchar(10),birth_date datetime,dead tinyint, death_date datetime, person_name varchar(500),phone_number varchar(100),identifier varchar(50), health_center int, index encounter_id (encounter_id), index person_enc (person_id,encounter_datetime), index enc_date_created (enc_date_created), index obs_date_created (obs_date_created), index ltfu (location_id,encounter_datetime));
...
#select @last_enc_date_created := '2014-07-23';
...
#select @last_obs_date_created := '2014-07-23';
...
(select * from amrs.obs o use index (date_Created) where concept_id in (1285,1946,5096,1502,1777,1088,1255,1250,9082,1596) and o.voided=0 and date_created > @last_obs_date_created);
...
(select 
	...
	min(if(concept_id=1596,value_coded,null)) as reason_exited_care,
...
(select person_id, group_concat(concat(`given_name`,' ',`middle_name`,' ',`family_name`) separator ' / ') as person_name
...
(select person_id,if(univ_identifier,univ_identifier,if(amrs_identifier,amrs_identifier,actg_identifier)) as identifier
from
(select 
	...
	min(if(identifier_type=27,identifier,null)) as actg_identifier
